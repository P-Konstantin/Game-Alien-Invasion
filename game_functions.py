...
def check_keydown_events(event, ai_settings, stats, screen, ship, bullets):
    ...
    # Загружаем звуковой эффект для выстрела
    sound_shot = pygame.mixer.Sound('music/ship_shot.mp3')
...
def ship_hit(ai_settings, screen, stats, sb, ship, aliens,
             bullets):
    """Обрабатывает стоклновение корабля с пришельцем."""
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens,
                  bullets):
    """
    Проверяет, достин ли флот края экрана,
    после чего обновялет позиции всех пришельцев во флоте.
    """
...
def start_game(ai_settings, stats):
    '''Запускает игру.'''
